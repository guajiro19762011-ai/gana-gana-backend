...
router.post('/comprar', async (req, res) => {
  ...
  try {
    const { data: usuario } = await supabase.from('usuarios').select('saldo').eq('id', usuario_id).single()
    if (!usuario || usuario.saldo < 5000)
    ...
    if (tomados && tomados.length > 0)
      return res.status(400).json({ error: `Los números ${tomados.map(t => t.numero).join(', ')} ya están tomados` })

    const { data: boleta, error: boletaError } = await supabase
      .from('boletas')
      .insert([{ usuario_id, sorteo_id: sorteo.id, numeros, valor: 5000 }])
      .select()
      .single()
    ...
    // Descontar $5000 y acreditar bono $500
    const nuevoSaldo = (usuario.saldo - 5000) + 500
    ...
    await supabase.from('movimientos').insert([{
      ...
      tipo: 'compra_boleta',
      monto: -5000,
      descripcion: 'Compra de boleta sorteo #' + sorteo.id,
      referencia_id: String(boleta.id)
    }])
    ...
    await supabase.from('movimientos').insert([{
      ...
      tipo: 'bono_compra',
      monto: 500,
      descripcion: 'Bono por compra de boleta #' + boleta.id,
      referencia_id: String(boleta.id)
    }])

    await supabase.from('sorteos').update({ total_boletas: sorteo.total_boletas + 1 }).eq('id', sorteo.id)

    if (sorteo.total_boletas + 1 >= 1000) {
      await supabase.from('sorteos').update({ estado: 'completo' }).eq('id', sorteo.id)
    }
...
router.get('/mis-boletas', async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('boletas')
      .select('*, sorteos(nombre, estado, numero_ganador)')
...
router.get('/resultados', async (req, res) => {
  try {
    ...
    if (!sorteo || !sorteo.numero_ganador) {
      return res.json({ sorteo: null, ganadores: [] })
    }
